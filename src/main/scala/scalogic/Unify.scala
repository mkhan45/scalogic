...
enum Term {
  ...
  case Const(v: Int)
  case Relation(name: String, args: List[String], body: Formula)
...
enum Formula {
  ...
  case Not(f: Formula)
  case RelApp(rel: Term.Relation, args: List[Term])
  ...
  def withSubst(v: String, `for`: Term): Formula = this match {
    ...
    case RelApp(rel, args) => RelApp(rel.withSubst(v, `for`).asInstanceOf[Term.Relation], args.map(_.withSubst(v, `for`)))
  }
