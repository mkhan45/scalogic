package scalogic.reunifysyntax
...
extension (e: Formula) {
  ...
  def ?(using facts: Set[Formula.Fact], relations: Map[String, Relation]): Res = {
    val res: Option[PartialSubsts] = e.solve
    res match {
      case Some(s: PartialSubsts) => {
        val substs: Option[Substs] = s.resolve
        val fvs = e.freeVars
        val fvSubsts = substs.map(r => fvs.map(v => (Term.Var(v), r.eval(Term.Var(v)))).toMap)
        Res(fvSubsts)
      }
      case None => Res(None)
    }
